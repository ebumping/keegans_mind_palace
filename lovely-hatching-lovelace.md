...
**Tasks:**
- [ ] Create `specs/audio-system.md` documenting desktop audio capture API, fallback strategy, and AudioAnalyser port from pale-strata
- [ ] Create `specs/room-generation.md` documenting procedural room algorithm, dimension ranges, doorway placement rules
- [ ] Create `specs/pattern-system.md` documenting pattern derivations from pale-strata (slicer, LFO, grain scheduling)
- [ ] Create `specs/shader-architecture.md` documenting audio uniform bindings, vertex displacement, fragment effects
- [ ] Create `specs/navigation.md` documenting first-person controls, collision detection, room transition triggers
- [ ] Create `specs/house-of-leaves.md` documenting liminal aesthetic reference (non-euclidean geometry, impossible spaces, disorientation)
- [ ] Create `specs/the-growl.md` documenting time-based dread system (hour thresholds, intensity scaling, effects)
- [ ] Create `specs/portal-variations.md` documenting MyHouse.wad-inspired alternate versions (5 escalating levels)
- [ ] Create `specs/circuitry-glitch.md` documenting circuit patterns and glitch animation triggers
...
- [ ] Configure TypeScript for strict mode with Three.js types
- [ ] Set up basic HTML template with full-viewport canvas
- [ ] Add Tailwind CSS for UI overlay elements
...
**Tasks:**
- [ ] Implement `AudioCapture` class using `getDisplayMedia({ audio: true })` for desktop audio
- [ ] Add fallback to `getUserMedia({ audio: true })` for microphone input
- [ ] Port AudioAnalyser from pale-strata with `getFrequencyData()`, `getTimeDomainData()`, `getLevels()`
- [ ] Add frequency band extraction (bass: 20-250Hz, mid: 250-4kHz, high: 4k-20kHz)
- [ ] Implement transient detection with configurable threshold
- [ ] Create React hook `useAudioAnalysis()` returning real-time levels
...
**Tasks:**
- [ ] Set up React Three Fiber canvas with dark purple background (`#1a1834`)
- [ ] Configure perspective camera with FOV 75, near 0.1, far 1000
- [ ] Add OrbitControls for initial development (disable for final)
- [ ] Implement fog for liminal depth effect (`FogExp2` with purple tint)
- [ ] Set up post-processing pipeline: Bloom, ChromaticAberration, Vignette
...
**Priority:** High
**Files:** `src/generators/RoomGenerator.ts`, `src/components/Room.ts`

**Tasks:**
- [ ] Create `RoomGenerator` class with configurable dimensions (width, height, depth)
- [ ] Implement non-euclidean room scaling based on audio levels (rooms breathe)
- [ ] Generate wall, floor, ceiling meshes with UV coordinates for patterns
- [ ] Add procedural doorway cutouts (variable count 1-4 based on seed)
- [ ] Implement room "complexity" parameter affecting subdivision density
...
**Tasks:**
- [ ] Port slicer pattern generation (8/16/32 step sequences) for texture animation
- [ ] Implement procedural stripe/grid patterns using sine wave combinations
- [ ] Create wallpaper-like repeating patterns with audio-modulated distortion
- [ ] Generate carpet patterns using grain scheduling intervals as seeds
- [ ] Add LFO-driven pattern morphing (sine, square, sawtooth, triangle, random)
...
**Tasks:**
- [ ] Create doorway frames with glowing edges (`#c792f5` accent)
- [ ] Implement infinite hallway illusion using portal-like rendering
- [ ] Generate hallway segments that extend/contract with bass frequencies
- [ ] Add door state: open, closed, partially open (audio-reactive)
- [ ] Create "impossible geometry" effects (hallways longer inside than outside)
...
**Tasks:**
- [ ] Create custom ShaderMaterial with audio uniform inputs (bass, mid, high, transient)
- [ ] Implement vertex displacement based on frequency data (walls ripple)
- [ ] Add fragment shader effects: color shifting, pattern distortion, glow pulsing
- [ ] Map transient detection to sudden visual "glitches" (color inversion, geometry spike)
- [ ] Implement pale-strata color interpolation for smooth state transitions
...
**Tasks:**
- [ ] Add floating dust particles that react to high frequencies
- [ ] Implement ambient occlusion that deepens with bass intensity
- [ ] Create light sources that flicker/pulse with mid frequencies
- [ ] Add shadow intensity modulation based on overall audio level
- [ ] Implement "breathing" scale animation (rooms expand/contract subtly)

---

### 3.3 Artifact Generation
**Priority:** Medium
**Files:** `src/components/Artifact.ts`, `src/generators/PatternGenerator.ts`

**Tasks:**
- [ ] Create procedural floating objects (geometric shapes, abstract forms)
- [ ] Implement artifact rotation/movement driven by audio phase
- [ ] Add artifacts that appear/disappear based on transient peaks
- [ ] Generate artifact patterns using pale-strata envelope curves
- [ ] Create "impossible objects" (Penrose-inspired) for surreal effect
...
**Tasks:**
- [ ] Create `timeStore` tracking deployment timestamp in localStorage
- [ ] Implement `getHoursSinceDeployment()` utility function
- [ ] Create GrowlSystem class managing ambient dread state (0-1 intensity)
- [ ] Add sub-bass drone audio that fades in over hours (starts inaudible)
- [ ] Implement shadow movement anomalies triggered by Growl intensity
- [ ] Add light flickering system scaled by Growl level
- [ ] Create "presence" effect - subtle camera shake, peripheral shadows
- [ ] Integrate Growl intensity with shader distortion uniforms
